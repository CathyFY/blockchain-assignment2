...
def connect_to_eth():
  ...
  url = "https://eth-mainnet.g.alchemy.com/v2/yl-cDg_cXKJw047BGQ05A35D63shBWL6"  # FILL THIS IN
...
def connect_with_middleware(contract_json):
  ...
  url_bnb = "https://bsc-testnet.public.blastapi.io"
  w3 = Web3(HTTPProvider(url_bnb))
  w3.middleware_onion.inject(ExtraDataToPOAMiddleware, layer=0)

	
  assert w3.is_connected(), f"Failed to connect to provider at {url_bnb}"
...
def is_ordered_block(w3, block_num):
  ...
  fees = [get_priority_fee(tx) for tx in transactions]
